...
class Codec:
    ...
    def deserialize(self, data):
        ...
        def helper(minimum, maximum):
            if preorder and minimum < preorder[0] < maximum:
                val = preorder.popleft()
                node = TreeNode(val)
                node.left = helper(minimum, val)
                node.right = helper(val, maximum)
                return node

        preorder = collections.deque(int(val) for val in data.split())
